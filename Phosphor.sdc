...
set_false_path -to [get_keepers {*media_native_audio:*|prefill_sync[0]}]
set_false_path -to [get_keepers {*media_native_audio:*|eof_sync[0]}]
...
# one rate family is active, but TimeQuest cannot infer that exclusivity from
# the hard clock-control mux, so describe both generated output clocks and
# mark them physically exclusive.
set music_44_master [get_clocks {*native_audio|clocks|cd_pll_44|*|divclk}]
set music_48_master [get_clocks {*native_audio|clocks|cd_pll_48|*|divclk}]
set music_mux_out [get_pins -compatibility_mode {*native_audio|clocks|selector|auto_generated|sd2|outclk}]
set music_44_mux_in [get_pins -compatibility_mode {*native_audio|clocks|selector|auto_generated|sd2|inclk[2]}]
set music_48_mux_in [get_pins -compatibility_mode {*native_audio|clocks|selector|auto_generated|sd2|inclk[3]}]
foreach collection [list $music_44_master $music_48_master $music_mux_out $music_44_mux_in $music_48_mux_in] {
    if {[get_collection_size $collection] != 1} {error "Native audio clock selector constraint must resolve exactly one node"}
}
create_generated_clock -name audio_mux_44 -master_clock $music_44_master -source $music_44_mux_in -divide_by 1 $music_mux_out
create_generated_clock -name audio_mux_48 -master_clock $music_48_master -source $music_48_mux_in -divide_by 1 -add $music_mux_out
set_clock_groups -physically_exclusive -group [get_clocks audio_mux_44] -group [get_clocks audio_mux_48]
...
set_false_path -to [get_keepers {*media_audio_clocks:*|cd44_gate_sync[0]}]
set_false_path -to [get_keepers {*media_audio_clocks:*|cd48_gate_sync[0]}]
set_false_path -from [get_keepers {*media_audio_clocks:*|cd48_gate_sync[2]}] -to [get_clocks audio_mux_44]
set_false_path -from [get_keepers {*media_audio_clocks:*|cd44_gate_sync[2]}] -to [get_clocks audio_mux_48]
