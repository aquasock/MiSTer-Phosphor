...
// mp3_dequant -> mp3_stereo -> mp3_antialias -- dumping every final
// spectral value for comparison against tools/mp3_antialias_reference.py
// (fed mp3_stereo_reference.py's own output) by sim/compare_antialias.py.
`timescale 1ns/1ps
module mp3_antialias_tb;
...
always @(posedge clk) begin
    if (out_valid) begin
        $display("AA gci=%0d idx=%0d data=%0d", out_gci, out_index, out_data);
        if (out_index == 10'd575) $display("AADONE gci=%0d", out_gci);
    end
    if (frame_done) begin
...
// This module lags mp3_stereo (already a multi-cycle-per-value stage)
// by another whole buffer+process+stream cycle, so the drain after the
// last frame needs to be at least as generous as mp3_stereo_tb.sv's, and
// for the same reason (a blocking delay sharing a block with the monitor
// above would miss any AA/AADONE events that arrive during it) it's kept
// in its own separate always block.
always @(posedge clk) begin
