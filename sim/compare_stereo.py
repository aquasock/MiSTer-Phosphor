#!/usr/bin/env python3
"""Cross-check mp3_stereo (the full frame_parser -> bit_reservoir ->
huffman_decoder -> dequant -> stereo chain) against
tools/mp3_stereo_reference.py's compute_stereo_fixed()/reorder_block(),
built on top of the already-validated mp3_dequant_reference.py output.
This check IS bit-exact: RTL and Python reference implement the identical
fixed-point algorithm, no float-vs-fixed ambiguity at this layer (unlike
requantization's own methodology note about FFmpeg -- see docs/MP3.md).

Usage: compare_stereo.py test_vectors/stereo_128k_44100.mp3 [num_frames]
"""
import re
import subprocess
import sys
...
def run_reference(mp3_path, num_frames):
    ...
    results = {}  # (frame_idx, gci) -> 576 final values
    for fi, (abs_pos, mdb, f, stereo) in enumerate(positions):
        ...
        for gr in range(2):
            ...
            for ch in range(nch):
                g = gs[ch]
                gci = gr * 2 + ch
                out = reorder_block(xr[ch], g["block_type"], g["mixed_block_flag"],
...
def run_sim(mp3_path, num_frames):
    hex_path = "/tmp/_cmp_st.hex"
    subprocess.run(["python3", str(REPO / "tools/mp3_to_hex.py"), mp3_path, hex_path], check=True)
    sim_bin = "/tmp/_cmp_st_tb.out"
    subprocess.run(
        [
            "iverilog", "-g2012", "-o", sim_bin,
            f"-Pmp3_stereo_tb.HEX_FILE=\"{hex_path}\"",
            f"-Pmp3_stereo_tb.NUM_FRAMES={num_frames}",
            str(REPO / "sim/mp3_stereo_tb.sv"),
...
def parse_sim(text):
    ...
    for line in text.splitlines():
        ...
        if m:
            ...
            cur[gci][i] = d
            continue
        m = re.match(r"STDONE gci=(\d)", line)
        if m:
            gci = int(m[1])
...
def compare(mp3_path, num_frames):
    ...
    if mismatches == 0:
        print(f"{mp3_path}: {checked} granule/channel stereo+reorder decodes match exactly (all 576 values)")
        return 0
