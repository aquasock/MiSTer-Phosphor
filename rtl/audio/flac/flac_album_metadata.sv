...
module flac_album_metadata(
 ...
 always @(posedge write_clk) begin
  if(reset||new_file) begin
   ...
   block_type<=0;block_last<=0;app_candidate<=0;app_good<=0;
   valid<=0;artwork_valid<=0;track_count<=0;title_long<=0;
   ...
   case(state)
    ...
    BODY:begin
     if(app_candidate)begin
      ...
      if(block_position>=103&&block_position<=3263&&block_position[4:0]==5'd7)
       title_long[(block_position-103)>>5]<=byte_data[0];
      case(block_position)
       0:if(byte_data!="M")app_good<=0;
       1:if(byte_data!="P")app_good<=0;
       2:if(byte_data!="3")app_good<=0;
       3:if(byte_data!="A")app_good<=0;
       4:if(byte_data!=1)app_good<=0;
